// Forzar ejecución inmediata con múltiples métodos de diagnóstico
// IMPORTANTE: Este script DEBE ejecutarse cuando se carga
(function() {
  'use strict';
  
  // Log inmediato ANTES de cualquier otra cosa - usar múltiples métodos
  if (typeof console !== "undefined") {
    console.log("%c[Components] ⚡ components.js cargado - INICIO", "color: green; font-weight: bold;");
    console.log("[Components] Timestamp:", new Date().toISOString());
    console.log("[Components] window.SitecComponents antes:", typeof window.SitecComponents);
    console.log("[Components] window existe:", typeof window !== "undefined");
    console.log("[Components] document existe:", typeof document !== "undefined");
    console.log("[Components] Script ejecutándose en:", window.location.href);
  }
  
  // También usar alert como fallback si console no funciona (solo en desarrollo)
  if (window.location.hostname === "127.0.0.1" || window.location.hostname === "localhost") {
    // No usar alert en producción, solo para debugging extremo
    // console.warn("[Components] Debug: Si ves esto, el script se está ejecutando");
  }
  ...
  function createSignaturePad(targetInput, labelText, options = {}) {
    ...
    if (targetInput.value && targetInput.value.startsWith("data:image")) {
      const img = new Image();
      img.onload = () => {
        // Dibujar en el siguiente frame para que el canvas ya esté en el DOM (evita que se borre)
  ...
  console.log("[Components] ✅ SitecComponents exportado correctamente");
  console.log("[Components] Timestamp:", new Date().toISOString());
  console.log("[Components] createField disponible:", typeof window.SitecComponents.createField === "function");
  console.log("[Components] window.SitecComponents después:", typeof window.SitecComponents);
  
  // Verificar que realmente se asignó
  if (!window.SitecComponents) {
    console.error("[Components] ❌ ERROR: window.SitecComponents es undefined después de asignación");
  } else {
    console.log("[Components] ✅ window.SitecComponents asignado correctamente");
  }
  } catch (error) {
    console.error("[Components] ❌ ERROR FATAL en components.js:", error);
    console.error("[Components] Stack trace:", error.stack);
    console.error("[Components] Error name:", error.name);
    console.error("[Components] Error message:", error.message);
    
    // Intentar asignar un objeto vacío para que el wizard no falle completamente
    if (!window.SitecComponents) {
      console.warn("[Components] ⚠️ Asignando SitecComponents vacío debido a error");
      window.SitecComponents = {
        labelFromName: function(name) { return name; },
        createField: function() { 
          console.error("[Components] createField no disponible debido a error de carga");
          return document.createElement("div");
        },
        createSignaturePad: function() { return document.createElement("div"); },
        createEvidenceUploader: function() { return document.createElement("div"); },
        createGeoPicker: function() { return document.createElement("div"); },
        createRiskMatrix: function() { return document.createElement("div"); },
        createGanttLite: function() { return document.createElement("div"); },
        createKanbanBoard: function() { return document.createElement("div"); },
        initAdvancedComponents: function() {},
        startAdvancedComponents: function() {},
        contracts: {}
      };
    }
