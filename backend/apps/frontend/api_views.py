...
class WizardValidateView(APIView):
    ...
    def post(self, request):
        ...
        if step == 1:
            ...
            # Validación: Si hay incidentes reportados, se requiere detalle
            incidents = data.get("incidents")
            # Manejar diferentes formatos: boolean, string, número
            has_incidents = (
                incidents is True or 
                incidents == "true" or 
                str(incidents).lower() == "true" or
                (isinstance(incidents, (int, float)) and incidents > 0)
            )
            
            if has_incidents:
                incidents_detail = data.get("incidents_detail")
                if not incidents_detail or (isinstance(incidents_detail, str) and not incidents_detail.strip()):
                    critical.append("incidents_detail_required")
            
            # Validación: Si hay incidentes Y la severidad es alta, se requiere plan de mitigación
            # Solo requerir mitigation_plan si hay incidentes reportados Y la severidad es alta
            incidents_severity = data.get("incidents_severity")
            if has_incidents and incidents_severity and str(incidents_severity).lower() == "high":
                mitigation_plan = data.get("mitigation_plan")
                if not mitigation_plan or (isinstance(mitigation_plan, str) and not mitigation_plan.strip()):
...
class WizardSyncView(APIView):
    ...
    def post(self, request):
        ...
        # Asegurar que resolution sea un diccionario (manejar None, {}, o cualquier otro tipo)
        ...
        for step_payload in incoming:
            ...
            # Ahora resolution siempre es un dict, así que podemos usar .get() de forma segura
            ...
            if existing and existing.updated_at and data.get("updatedAt"):
                ...
                if client_dt and client_dt < existing.updated_at:
                    # Solo crear conflicto si realmente hay diferencias en los campos
                    field_conflicts = self._build_field_conflicts(existing.data, data)
                    # Si no hay conflictos de campos, usar la versión del servidor automáticamente
                    if not field_conflicts:
                        # Los datos son idénticos, solo el timestamp es diferente
                        # Usar la versión del servidor automáticamente
                        updated.append(WizardStepDataSerializer(existing).data)
                        continue
                    # Hay conflictos reales en los campos
                    conflicts.append(
                        {
                            "step": step,
...
class VerifyPasswordView(APIView):
    ...
    def post(self, request):
        ...
        if not password:
            return Response(
                {"verified": False, "error": "Contraseña requerida"}, 
                status=status.HTTP_400_BAD_REQUEST
            )

        # Verificar contraseña directamente usando check_password
        # Esto es más confiable que authenticate() cuando el usuario ya está autenticado
        user = request.user
        if user.check_password(password):
            log_audit_event(request, "reauthentication_success", user)
            return Response({"verified": True})
        else:
            log_audit_event(request, "reauthentication_failed", user)
            return Response(
                {"verified": False, "error": "Contraseña incorrecta"},
