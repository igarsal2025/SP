...
from .services import check_ai_throttle, record_ai_usage, estimate_ai_cost
...
class AiSuggestView(APIView):
    ...
    def post(self, request):
        ...
        allowed, reason, retry_after = check_ai_throttle(request.company, request.user, mode)
        if not allowed:
            return Response(
                {
                    ...
                    "message": f"Límite de requests de IA excedido. Intente nuevamente en {retry_after} segundos.",
                },
        ...
        # Registrar uso y costo estimado
        cost_estimate = estimate_ai_cost(mode, model_name, latency_ms)
        record_ai_usage(request.company, request.user, mode, cost_estimate)

        log_audit_event(request, "ai_suggest", record, extra_data={"mode": mode, "cost_estimate": cost_estimate})
