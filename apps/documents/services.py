...
def request_nom151_stamp(document, checksum_sha256):
    ...
    if not provider_url:
        # Proveedor NOM-151 no configurado - sistema funciona normalmente sin timbrado
        logging.getLogger(__name__).info("NOM151 provider no configurado - documento generado sin timbrado real")
        return "pendiente", {
            "latency_ms": 0,
            "attempts": 0,
            "success": False,
            "status": "disabled",
            "message": "Proveedor NOM-151 no configurado. El documento se genera correctamente sin timbrado real."
        }
